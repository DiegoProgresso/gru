...
const especificacoes = {
    pequena: {
        tipo: "transporte",
        titulo: "Carga pequena",
        descricao: "Informe origem, destino, volume e peso para encontrarmos um espaço compatível."
    },
    moveis: {
        tipo: "transporte",
        titulo: "Móveis e mudanças",
        descricao: "Informe origem, destino, volume e peso da mudança."
    },
    empresarial: {
        tipo: "transporte",
        titulo: "Carga empresarial",
        descricao: "Informe origem, destino, volume e peso da mercadoria."
    },
    armazenamento: {
        tipo: "armazem",
        titulo: "Armazenamento",
        descricao: "Informe a localização e a capacidade necessária para armazenar seus produtos."
    }
};

let pesquisaTipoAtual = "transporte";


function abrirPesquisa(chave) {

    const dado = especificacoes[chave];

    if (!dado) {
        return;
    }

    pesquisaTipoAtual = dado.tipo;

    document.getElementById("pesquisaTitulo").textContent = dado.titulo;
    document.getElementById("pesquisaDescricao").textContent = dado.descricao;

    const ehArmazem = dado.tipo === "armazem";

    document.getElementById("pesqCampoDestino")
        .classList.toggle("hidden", ehArmazem);

    document.getElementById("pesqCampoPeso")
        .classList.toggle("hidden", ehArmazem);

    document.getElementById("pesqLabelDestino").textContent =
        ehArmazem ? "Localização" : "Destino";

    document.getElementById("pesqLabelVolume").textContent =
        ehArmazem ? "Capacidade necessária (m³)" : "Volume (m³)";

    document.getElementById("pesqResultado").innerHTML = "";

    abrirAba("pesquisa");
}


function pesquisarEspecificacao() {

    const origem =
        document.getElementById("pesqOrigem").value.trim();

    const destino =
        document.getElementById("pesqDestino").value.trim();

    const volume =
        document.getElementById("pesqVolume").value.trim();

    const prazo =
        document.getElementById("pesqPrazo").value;

    const resultado =
        document.getElementById("pesqResultado");

    const ehArmazem = pesquisaTipoAtual === "armazem";

    const campoLocal = ehArmazem ? origem : origem;
    const campoDestinoNecessario = ehArmazem ? true : !!destino;

    if (!campoLocal || !campoDestinoNecessario || !volume || !prazo) {

        resultado.innerHTML = `
            <div class="alerta">
                Preencha todos os campos para pesquisar.
            </div>
        `;
    ...
    const termoBusca = (origem + " " + destino).trim();

    abrirAba("mapa");

    setTimeout(() => {

        document.getElementById("buscaMapa").value = termoBusca;
        ...
        document
            ...
            .forEach(btn => {
                btn.classList.toggle(
                    "ativo",
                    btn.dataset.tipo === pesquisaTipoAtual
                );
            });

        const filtrados = locais.filter(local => {

            const combinaTipo = local.tipo === pesquisaTipoAtual;

            const combinaBusca =
                !termoBusca ||
                local.cidade.toLowerCase().includes(termoBusca.toLowerCase()) ||
                local.destino.toLowerCase().includes(termoBusca.toLowerCase()) ||
                local.titulo.toLowerCase().includes(termoBusca.toLowerCase());

            return combinaTipo && combinaBusca;
        });

        mostrarMarcadores(filtrados.length ? filtrados : locais.filter(l => l.tipo === pesquisaTipoAtual));

        if (filtrados.length) {
            mapa.setView([filtrados[0].lat, filtrados[0].lng], 7);
        }
